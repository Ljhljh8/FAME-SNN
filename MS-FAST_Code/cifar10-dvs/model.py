...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj1_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=1.0, detach_reset=True,
                                          backend='cupy')
        ...
        self.proj2_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=1.0, detach_reset=True,
                                          backend='cupy')
        ...
        self.proj3_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=1.0, detach_reset=True,
                                          backend='cupy')
        ...
        self.proj4_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=1.0, detach_reset=True,
                                          backend='cupy')
        ...
        self.proj5_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=1.0, detach_reset=True,
                                          backend='cupy')
    ...
    def forward(self, x, alpha):
        ...
        x = self.proj_bn(x)
        x = (self.maxpool(x)).reshape(T, B, -1, 64, 64)

        x = self.proj1_lif(x, alpha).flatten(0, 1).contiguous()
        x = self.proj1_conv(x)
        x = self.proj1_bn(x)
        x = (self.maxpool1(x)).reshape(T, B, -1, 32, 32)

        x = self.proj2_lif(x, alpha).contiguous()
        FPN_input.append(x)
        x = self.proj2_conv(x.flatten(0, 1))
        x = self.proj2_bn(x)
        x = (self.maxpool2(x)).reshape(T, B, -1, 16, 16)

        x = self.proj3_lif(x, alpha).contiguous()
        FPN_input.append(x)
        x = self.proj3_conv(x.flatten(0, 1))
        x = self.proj3_bn(x)
        x = (self.maxpool2(x)).reshape(T, B, -1, H // 16, W // 16)

        x = self.proj4_lif(x, alpha).contiguous()
        FPN_input.append(x)
        x = self.proj4_conv(x.flatten(0, 1))
        x = self.proj4_bn(x).reshape(T, B, -1, H // 16, W // 16)
        x = self.proj5_lif(x, alpha).contiguous()
        FPN_input.append(x)
...
class vit_snn(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=16, pretrained_cfg=None, TET=None,
                 ):
        super().__init__()
        self.iter_num = 0
        self.step_size = 281
    ...
    def forward(self, x):
        if self.training:
            ...
            if self.iter_num % self.step_size == 0:
                if (1.0 - self.iter_num / (self.STH_epoch * 562)) > 0.0:
                    self.alpha = math.pow((1.0 - self.iter_num / (self.STH_epoch * 562)), 2.0)
                else:
                    self.alpha = 0.0
                print(self.iter_num)
                print(self.alpha)
