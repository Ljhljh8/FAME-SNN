...
def parse_args():
    ...
    parser.add_argument('--model-name', default='train', help='model')
    parser.add_argument('--dataset', default='DVSUCF101', help='dataset')
    parser.add_argument('--num-classes', type=int, default=101, metavar='N',
                        help='number of label classes (default: 101)')
    parser.add_argument('--data-path', default='/home/think/ucf101_dvs', help='dataset')
    ...
    parser.add_argument('--opt', default='adamp', type=str, metavar="OPTIMIZER", help='Optimizer (default: "adamp")')
...
def main(args):
    ...
    output_dir_txt = os.path.join(output_dir, f'lr{args.model_name}')
    output_dir = os.path.join(output_dir, f'lr{args.lr}')
    if not os.path.exists(output_dir):
        utils.mkdir(output_dir)
    if not os.path.exists(output_dir_txt):
        utils.mkdir(output_dir_txt)
        ...
        if output_dir:
            ...
            if save_max:
                utils.save_on_master(
                    checkpoint,
                    os.path.join(output_dir, 'checkpoint_max_test_{}.pth'.format(max_test_acc1)))
        with open(os.path.join(output_dir_txt, 'epoch_result_.txt'), 'a') as f:
            f.write(
                f"TIME: {time.time()}, End_Epoch: {epoch}, Acc1: {test_acc1} , max ACC1:{max_test_acc1}\n")
        print(args)
