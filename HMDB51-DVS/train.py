...
parser.add_argument('-c', '--config', default='HMDB51DVS.yml', type=str, metavar='FILE',
                    help='YAML config file specifying default arguments')

parser = argparse.ArgumentParser(description='PyTorch cifar10dvs Training')

# Model detail
parser.add_argument('--model', default='MS-FAST', type=str, metavar='MODEL',
                    help='Name of model to train (default: "countception"')
parser.add_argument('-T', '--time-step', type=int, default=16, metavar='time',
                    help='simulation time step of spiking neuron (default: 4)')
parser.add_argument('-L', '--layer', type=int, default=2, metavar='layer',
...
parser.add_argument('--resume', default='/media/think/data1/ljh/FAME-SNN/HMDBdvs/output/train/20250121-152545-MS_FAST-224/model_best.pth.tar', type=str, metavar='PATH',
...
parser.add_argument("--Test_only",dest="test_only",default=False,help="Only test the model",action="store_true")
...
def unpack_mix_param(args):
    ...
    prob = args['prob'] if 'prob' in args else .6
    num = args['num'] if 'num' in args else 1
    num_classes = args['num_classes'] if 'num_classes' in args else 10
    noise = args['noise'] if 'noise' in args else 0.1
    gaussian_n = args['gaussian_n'] if 'gaussian_n' in args else 0.1
    return mix_up, cut_mix, event_mix, beta, prob, num, num_classes, noise, gaussian_n


def get_HMDBDVS_data(batch_size, T, **kwargs):
    sensor_size = HMDBDVS.sensor_size

    train_transform = transforms.Compose([
    # tonic.transforms.ToFrame(time_window=120000, sensor_size=sensor_size),
        tonic.transforms.ToFrame(n_time_bins=T, sensor_size=sensor_size),
    ])
    ...
    train_dataset = HMDBDVS('/home/think/ljh_datasets/HMDBDVS/train/', transform=train_transform)
    # 1
    test_dataset = HMDBDVS('/home/think/ljh_datasets/HMDBDVS/test/', transform=test_transform)

    train_transform = transforms.Compose([
        to_float_tensor,
        transforms.Resize((224, 224), antialias=True),
        # transforms.RandomCrop((176, 176)),
    ...
    train_dataset = DiskCachedDataset(train_dataset,
                                      cache_path='/home/think/ljh_datasets/HMDBDVS/train_cache/',
                                      transform=train_transform)
    test_dataset = DiskCachedDataset(test_dataset,
                                     cache_path='/home/think/ljh_datasets/HMDBDVS/test_cache/',
...
def main():
    setup_default_logging(log_path=f"./logs/training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    model = create_model(
        'FAME_SNN',
        pretrained=False,
        drop_mlp=args.drop_mlp, attn_drop_rate=args.attn_drop_rate, drop_path_rate=args.drop_path_rate, drop_block_rate=None,
        img_size_h=args.img_size, img_size_w=args.img_size, patch_size=args.patch_size,
        embed_dims=args.dim, num_heads=args.num_heads, mlp_ratios=args.mlp_ratio,
        in_channels=2, num_classes=args.num_classes, depths=args.depths, sr_ratios=1, T=args.time_step,
        STHOS=args.STHOS, alpha_init=args.alpha_init
    )
    ...
    loader_train, loader_eval, mixup_active = get_HMDBDVS_data(args.batch_size, T=8)
    ...
    #     train_interpolation = data_config['interation']
    #
    ...
    #         sampler=train_sampler,
    #         num_workers=4,  # ������Ļ�������
    #         pin_memory=True,  # ʹ��GPUʱ��������pin_memory
    #     )
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics = train_one_epoch(
                ...
                mixup_fn=mixup_fn, aug=train_snn_aug, trival_aug=train_trivalaug)
    ...
    # using real data
    Nops, Nparams = get_model_complexity_info(model, (2, 224, 224), loader_eval, as_strings=True,
                                              print_per_layer_stat=True, verbose=True, syops_units='Mac',
                                              param_units=' ', output_precision=3)
    # using random input
    #Nops, Nparams = get_model_complexity_info(model, (3,  32, 32), dataloader=None, as_strings=True, print_per_layer_stat=True, verbose=True, syops_units='Mac', param_units=' ', output_precision=3)
    print("Nops: ", Nops)
...
def train_one_epoch(
        ...
        loss_scaler=None, model_ema=None, mixup_fn=None, aug=None, trival_aug=None, ):
    if args.mixup_off_epoch and epoch >= args.mixup_off_epoch:
        # if args.prefetcher and loader.collate_fn.mixup_enabled:
        #     loader.collate_fn.mixup_enabled = False
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        input = input.float()  # [N, T, C, H, W]
        N, T, C, H, W = input.shape
        if aug != None:
            # image = image.flatten(1, 2).contiguous() # vT,C
            input = torch.stack([(aug(input[i])) for i in range(N)])
            # image = image.reshape(N,T,C,H,W)

        if trival_aug != None:
            # image = image.flatten(0,1).contiguous()
            input = torch.stack([(trival_aug(input[i])) for i in range(N)])
            # image = image.reshape(N,T,C,H,W).contiguous()
        # if mixup_fn is not None:
        #     # image = image.flatten(1, 2).contiguous() # vT,C
        #     image, target = mixup_fn(input, target)
        ...
        with amp_autocast():
            output = model(input, len(loader), epoch)
            # target = torch.nn.functional.one_hot(target,args.num_classes)
            loss = loss_fn(output, target)

        pred_choice = output.data.max(1)[1]
        labe = target
        correct = pred_choice.eq(labe.long().data).cpu().sum()
        mean_correct.append(correct.item() / float(input.size()[0]))
        ...
        spikingjelly.clock_driven.functional.reset_net(model)
    ...
    return OrderedDict([('loss', losses_m.avg), ('acc', train_instance_acc)])
...
def validate(model, loader, loss_fn, args, amp_autocast=suppress, log_suffix=''):
    ...
    metrics = OrderedDict([ ('top1', top1_m.avg), ('top5', top5_m.avg), ('loss', losses_m.avg),])
