...
class ASFF(nn.Module):
    def __init__(self, dim, num_heads=8, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.bn0_r = nn.BatchNorm2d(dim)
        self.bn0_i = nn.BatchNorm2d(dim)
    ...
    def forward(self, x, alpha):
        ...
        x.real = self.bn0_r(x.real.flatten(0, 1)).reshape(T, B, C, H, W)
        x.imag = self.bn0_i(x.imag.flatten(0, 1)).reshape(T, B, C, H, W)
...
class MS_STF(nn.Module):
    ...
    def forward(self, x, alpha):
        ...
        tempro_fusion = self.MSTF_lif[-1](add_pre2corent.reshape(T//k[-1], k[-1]*B, -1, H, W), alpha)
        ...
        for i in range(len(x) - 2, -1, -1):
            ...
            tempro_fusion = self.MSTF_lif[i](tempro_fusion.reshape(T//k[i], k[i]*B, -1, H, W), alpha)
            tempro_fusion = tempro_fusion.reshape(T, B, -1, H, W)

            tempro_fusion = tempro_fusion.permute(3, 4, 1, 0, 2).contiguous()   # H, W, B, T, C
            tempro_fusion = self.MSTF[i](tempro_fusion.flatten(0, 2))
            head_output.append(tempro_fusion)
        x = torch.stack((list((head_output))), dim=0).reshape(-1, H, W, B, T, C)
