...
class ASFF(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        self.act01_r = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=0.5, detach_reset=True, backend='cupy')
        self.act01_i = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=0.5, detach_reset=True, backend='cupy')
        self.act11 = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=0.5, detach_reset=True, backend='cupy')

        self.act12 = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=0.5, detach_reset=True, backend='cupy')
        self.act31 = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=0.5, detach_reset=True, backend='cupy')
        self.act32 = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=0.5, detach_reset=True, backend='cupy')
        ...
        self.act21 = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=0.5, detach_reset=True, backend='cupy')
        self.act22 = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=0.5, detach_reset=True, backend='cupy')
    ...
    def forward(self, x, alpha):
        ...
        origin_ffted_o = self.act21(x.real, alpha)
        origin_ffted_e = self.act22(x.imag, alpha)
        x.real = origin_ffted_o
        x.imag = origin_ffted_e
        ...
        o2_p = o2_real - o2_imag
        o2_n = o2_real + o2_imag

        x = o2_p * origin_ffted_o - (o2_n * origin_ffted_e)
...
class MS_STF(nn.Module):
    def __init__(self, in_channel_list, out_channel):
        ...
        self.MSTF_lif = nn.ModuleList(
            [neuron.STHLIFNode(step_mode='m', tau=(tau_thr + 0.5 * t_i), v_threshold=1.0, detach_reset=True, backend='cupy')
             for t_i,_ in enumerate(in_channel_list) ])
    ...
    def forward(self, x, alpha):
        ...
        x_res = add_pre2corent.reshape(T, B, C, H, W)
        tempro_fusion = self.MSTF_lif[-1](x_res, alpha)
        ...
        # x = torch.stack((list((head_output))), dim=0).reshape(-1, T, B, C, H, W)
        # x = torch.mean(x, dim=0)
        return x
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj1_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, detach_reset=True, backend='cupy')
        ...
        self.proj2_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, detach_reset=True, backend='cupy')
        ...
        self.proj3_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, detach_reset=True, backend='cupy')
        ...
        self.proj4_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, detach_reset=True, backend='cupy')
        ...
        self.proj5_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, detach_reset=True, backend='cupy')

        self.Top_down = MS_STF([embed_dims//4, embed_dims//2, embed_dims, embed_dims], embed_dims)
...
class vit_snn(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=8, pretrained_cfg=None,
                 ):
        super().__init__()
    ...
    def forward(self, x):
        if self.training:
            ...
            if self.iter_num % self.step_size == 0:
                ...
                if (1.0 - self.iter_num / (40 * 394.0)) > 0.0:
                    # self.alpha = 0.0
                    self.alpha = math.pow((1.0 - self.iter_num / (40 * 394.0)), 2.0)
                    print(self.iter_num)
                    print(self.alpha)
                else:
                    self.alpha = 0.0
