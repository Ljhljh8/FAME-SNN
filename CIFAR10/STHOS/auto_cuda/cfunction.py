...
def qlsigmoid_backward(y: str, real_spike_seq:str, alpha: float, dtype: str):
    assert y is not None
    # codes = sigmoid(y=f'const {dtype} sigmoid_backward__sigmoid_ax', x=x, alpha=alpha, dtype=dtype) + '\n'
    alpha = constant(None, alpha, dtype)
    if dtype == 'float':
        codes = f'{y} = (1.0f - {real_spike_seq}) * {real_spike_seq} * {alpha};'
        return codes
    elif dtype == 'half2':
        codes = f'{y} = __hmul2(__hmul2(__hsub2(__float2half2_rn(1.0f), {real_spike_seq}), {real_spike_seq}), {alpha});'
        return codes
    else:
        raise NotImplementedError(dtype)

def alpha_mix(z: str or None, x: str, y: str, alpha: str, dtype: str):
    # alpha 1 --> 0
    # x = spike y = real_spike
    # z = mix_spike = alpha * real_spike + (1-alpha) * spike
    if dtype == 'float':
        ...
        if x == '0.0f':
            ...
            codes = f'{x} * (1. - {alpha}) + {y} * {alpha}'

    elif dtype == 'half2':
        if x == '__float2half2_rn(0.0f)':
            codes = f'__hmul2({y}, {alpha})'

        elif y == '__float2half2_rn(0.0f)':
            codes = f'__hmul2({x}, __hsub2(__float2half2_rn(1.0f), {alpha}))'
        else:
            codes = f'__hadd2(__hmul2({y}, {alpha}), __hmul2({x}, __hsub2(__float2half2_rn(1.0f), {alpha})))'
    else:
        raise NotImplementedError(dtype)
