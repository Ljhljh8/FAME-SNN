...
class STHLIFNode(BaseNode):
    ...
    def jit_eval_multi_step_forward_hard_reset_decay_input(x_seq: torch.Tensor, v: torch.Tensor, v_threshold: float,
                                                           v_reset: float, tau: float, alpha: float):
        spike_seq = torch.zeros_like(x_seq)
        # --------------add----------------------
        real_spike_seq = torch.zeros_like(x_seq)
        mix_spike_seq = torch.zeros_like(x_seq)
        # --------------add-----------------------
        for t in range(x_seq.shape[0]):
            v = v + (x_seq[t] - (v - v_reset)) / tau
            spike = (v >= v_threshold).to(x_seq)
            # ---------------------add---------------------------------
            real_spike_seq = 1.0 / (1.0 + math.exp(-4.0 * (v - v_threshold)))
            mix_spike_seq = alpha * real_spike_seq + (1 - alpha) * spike
            # ---------------------add---------------------------------
            v = v_reset * spike + (1. - spike) * v
            # spike_seq[t] = spike
            spike_seq[t] = mix_spike_seq
        return spike_seq, v
    ...
    def multi_step_forward(self, x_seq: torch.Tensor, alpha):
        if self.training:
            if self.backend == 'torch':
                ...
                if self.forward_kernel is None or not self.forward_kernel.check_attributes(hard_reset=hard_reset, dtype=dtype, decay_input=self.decay_input):
                    self.forward_kernel = ac_neuron_kernel.STHLIFNodeFPTTKernel(decay_input=self.decay_input, hard_reset=hard_reset, dtype=dtype)
                ...
                if self.backward_kernel is None or not self.backward_kernel.check_attributes(
                    ...
                    self.backward_kernel = ac_neuron_kernel.STHLIFNodeBPTTKernel(decay_input=self.decay_input, surrogate_function=self.surrogate_function.cuda_codes, hard_reset=hard_reset, detach_reset=self.detach_reset, dtype=dtype)

                self.v_float_to_tensor(x_seq[0])

                spike_seq, v_seq = ac_neuron_kernel.STHLIFNodeATGF.apply(x_seq.flatten(1), alpha, self.v.flatten(0),
