...
from diversebranchblock import DiverseBranchBlock1D, DiverseBranchBlock2D
class MLP(nn.Module):
...
class SepConv(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        med_channels = int(expansion_ratio * out_dim)
        # self.lif1 = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=self.ASSF_th, detach_reset=True, backend='cupy')

        self.pwconv1 = nn.Conv2d(in_dim, med_channels, kernel_size=1, stride=1, bias=bias)
        self.bn1 = nn.BatchNorm2d(med_channels)
        self.lif2 = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=1.0, detach_reset=True, backend='cupy')
        ...
        self.pwconv2 = nn.Conv2d(med_channels, out_dim, kernel_size=1, stride=1, bias=bias)
...
class vit_snn(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=4, STHOS=None, pretrained_cfg=None,
                 ):
        super().__init__()
