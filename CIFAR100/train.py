...
# Model detail
parser.add_argument('--model', default='FAME_SNN', type=str, metavar='MODEL',
...
parser.add_argument('-data-dir', metavar='DIR',default="/home/think/cifar100_data/",
...
def main():
    ...
    model = create_model(
        ...
        img_size_h=args.img_size, img_size_w=args.img_size, T=args.time_step,
        patch_size=args.patch_size, embed_dims=args.dim, num_heads=args.num_heads,
        mlp_ratios=args.mlp_ratio, in_channels=3, num_classes=args.num_classes,
        qkv_bias=False, depths=args.depths, STHOS=args.STHOS, sr_ratios=1,
    )
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            len_loader = len(loader_train)
            train_metrics = train_one_epoch(
                epoch, model, loader_train, optimizer, train_loss_fn, args, len_loader,
            ...
            eval_metrics = validate(model, loader_eval, validate_loss_fn, args, len_loader, amp_autocast=amp_autocast)
...
def train_one_epoch(
        epoch, model, loader, optimizer, loss_fn, args,len_loader,
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        with amp_autocast():
            output = model(input, len_loader, epoch)
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            # print('Accuracy/Train Instance Accuracy', train_instance_acc, epoch)
            if args.distributed:
...
def validate(model, loader, loss_fn, args, len_loader,amp_autocast=suppress, log_suffix=''):
    batch_time_m = AverageMeter()
    losses_m = AverageMeter()
    top1_m = AverageMeter()
    top5_m = AverageMeter()
