...
class ASFF(nn.Module):
    ...
    def forward(self, x, alpha):
        ...
        dtype = x.dtype
        x = x.float()
        #x = self.bn0(x.flatten(0, 1)).reshape(T, B, C, H, W)
        x = self.act(x, alpha)
        ...
        x = x.type(dtype)
        #x = self.bn3_2(x.flatten(0, 1)).reshape(T, B, C, H, W)
        return x
...
class MS_STF(nn.Module):
    def __init__(self, in_channel_list, out_channel, multi_T, threshold, tauthr):
        super().__init__()
        ...
        self.MSSF = nn.ModuleList([nn.Sequential(nn.Conv2d(in_ch,in_ch,kernel_size=3,stride=1,padding=1,groups=in_ch,bias=False),
        ...
        self.MSTF = nn.ModuleList([nn.Sequential(
                    ...
                    nn.Conv1d(out_channel,out_channel,kernel_size=3,stride=1,padding=1,groups=out_channel,bias=False),
    ...
    def forward(self, x, alpha):
        ...
        tempro_fusion = tempro_fusion.view( H, W, B * self.multi_T[-1], -1, T // self.multi_T[-1]).permute(4,2,3,0,1).contiguous()
        ...
        for i in range(len(x) - 2, -1, -1):
            ...
            tempro_fusion = tempro_fusion.view( H, W, B * self.multi_T[i], -1, T // self.multi_T[i]).permute(4,2,3,0,1).contiguous()
            tempro_fusion = tempro_fusion.reshape(T, B, -1, H, W)
...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256, T=4):
        ...
        self.proj1_lif = neuron.STHLIFNode(step_mode='m', tau=self.tauthr[0], v_threshold=self.thresholds[0],
        ...
        self.proj2_lif = neuron.STHLIFNode(step_mode='m', tau=self.tauthr[1], v_threshold=self.thresholds[1],
                                           detach_reset=True, backend='cupy')
        ...
        self.proj3_lif = neuron.STHLIFNode(step_mode='m', tau=self.tauthr[2], v_threshold=self.thresholds[2],
                                           detach_reset=True, backend='cupy')
        ...
        self.proj4_lif = neuron.STHLIFNode(step_mode='m', tau=self.tauthr[3], v_threshold=self.thresholds[3],
                                           detach_reset=True, backend='cupy')
        self.proj4_conv = nn.Conv2d(embed_dims, int(embed_dims), kernel_size=3, stride=1, padding=1, bias=False)
        self.proj4_bn = nn.BatchNorm2d(int(embed_dims))
        self.proj4_mp = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
        self.proj5_lif = neuron.STHLIFNode(step_mode='m', tau=self.tauthr[4], v_threshold=self.thresholds[4],
                                           detach_reset=True, backend='cupy')

        self.MS_STF = MS_STF([embed_dims // 4, embed_dims // 2, embed_dims, int(embed_dims)], embed_dims,
                             self.multi_T, self.thresholds, self.tauthr)

    def forward(self, x, alpha):
        T, B, C, H, W = x.shape
        FPN_input = []
        x = self.proj_conv(x.flatten(0, 1))
        x = self.proj_bn(x).reshape(T // self.multi_T[0], self.multi_T[0] * B, -1, H, W)

        x = self.proj1_lif(x, alpha)
        x = x.reshape(T, B, -1, H, W)
        x = self.proj1_conv(x.flatten(0, 1))
        x = self.proj1_bn(x).reshape(T // self.multi_T[1], self.multi_T[1] * B, -1, H, W)
        ...
        x = self.proj2_bn(x)
        x = (self.proj2_mp(x)).reshape(T // self.multi_T[2], self.multi_T[2] * B, -1, int(H / 2), int(W / 2))

        x = self.proj3_lif(x, alpha)
        x = x.reshape(T, B, -1, int(H / 2), int(W / 2))
        FPN_input.append(x)
        x = self.proj3_conv(x.flatten(0, 1))
        x = self.proj3_bn(x)
        x = (self.proj3_mp(x)).reshape(T // self.multi_T[3], self.multi_T[3] * B, -1, int(H / 4), int(W / 4))

        x = self.proj4_lif(x, alpha)
        x = x.reshape(T, B, -1, int(H / 4), int(W / 4))
        FPN_input.append(x)
        x = self.proj4_conv(x.flatten(0, 1))
        x = self.proj4_bn(x).reshape(T // self.multi_T[4], self.multi_T[4] * B, -1, int(H / 4), int(W / 4))
...
class vit_snn(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=4, STHOS=None, pretrained_cfg=None,pretrained_cfg_overlay=None
                 ):
        super().__init__()
        ...
        #self.head = SGLFormerClassificationHead(channels=embed_dims, height=self.H, width=self.W,time_steps=self.T, num_classes=num_classes)
    ...
    def forward(self, x, len_loader=390, epoch=0):
...
if __name__ == '__main__':
    ...
    model = create_model(
        ...
        patch_size=8, embed_dims=384, num_heads=8, mlp_ratios=4,
