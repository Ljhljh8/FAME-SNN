...
class MS_STF(nn.Module):
    ...
    def forward(self, x, alpha):
        ...
        for i in range(len(x) - 2, -1, -1):
            ...
            tempro_fusion = self.MSTF_lif[-1](tempro_fusion.reshape(T, B, C, H, W), alpha)
            tempro_fusion = tempro_fusion.permute(3, 4, 1, 2, 0).contiguous()
            tempro_fusion = self.MSTF[-1](tempro_fusion.flatten(0, 2))
            head_output.append(tempro_fusion)
...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.proj1_lif = neuron.STHLIFNode(step_mode='m',tau=tau_thr, v_threshold=1.0, detach_reset=True, backend='cupy')
        ...
        self.proj2_lif = neuron.STHLIFNode(step_mode='m',tau=tau_thr, v_threshold=1.0, detach_reset=True, backend='cupy')
        ...
        self.proj3_lif = neuron.STHLIFNode(step_mode='m',tau=tau_thr, v_threshold=1.0, detach_reset=True, backend='cupy')
        ...
        self.proj4_lif = neuron.STHLIFNode(step_mode='m',tau=tau_thr, v_threshold=1.0, detach_reset=True, backend='cupy')
        ...
        self.proj5_lif = neuron.STHLIFNode(step_mode='m', tau=tau_thr, v_threshold=1.0, detach_reset=True, backend='cupy')
...
class vit_snn(nn.Module):
    def __init__(self,
                 ...
                 depths=[6, 8, 6], sr_ratios=[8, 4, 2], T=16, pretrained_cfg=None,qlepoch = 10
                 ):
        super().__init__()
        self.iter_num = 0
        self.step_size = 24
    ...
    def forward(self, x):
        if self.training:
            ...
            if self.iter_num % self.step_size == 0:
                if (1.0 - self.iter_num / (self.STH_epoch * 73)) > 0.0:
                    self.alpha = math.pow((1.0 - self.iter_num / (self.STH_epoch * 73)), 2.0)
                else:
                    self.alpha = 0.0
                print(self.iter_num)
                print(self.alpha)
