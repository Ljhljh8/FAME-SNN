...
def neuronal_sigmoid(real_spike: str, v: str, v_th: str, dtype: str = 'float'):
    if dtype == 'float':
        return cfunction.sigmoid(y=real_spike, x=f'({v} - {v_th})',alpha=4.0 ,dtype=dtype)
    elif dtype == 'half2':
        return cfunction.sigmoid(y=real_spike, x=f'__hsub2({v}, {v_th})',alpha=4.0, dtype=dtype)
    else:
        raise NotImplementedError(dtype)

def neuronal_mix(mix_spike:str, spike: str, real_spike: str, alpha: str, dtype: str = 'float'):
    if dtype == 'float':
        return cfunction.alpha_mix(z=mix_spike, x=spike, y=real_spike, alpha=alpha, dtype= dtype)
    elif dtype == 'half2':
        return cfunction.alpha_mix(z=mix_spike, x=spike, y=real_spike, alpha=alpha, dtype= dtype)
    else:
        raise NotImplementedError(dtype)
...
class STHNeuronFPTTKernel(base.CKernel2D):
    def __init__(self, hard_reset: bool, dtype: str):
        ...
        # ----------------add--------------------------------
        self.add_param(ctype=f'{dtype} *', cname='mix_spike_seq')
        self.add_param(ctype=f'{dtype} *', cname='real_spike_seq')
        self.add_param(ctype=f'{dtype} &', cname='alpha')
        # ----------------add--------------------------------
    ...
    def core(self):
        ...
        # -------------------add------------------------
        core_codes.append(neuronal_sigmoid(real_spike='real_spike_seq[t]', v='h_seq[t]', v_th='v_th', dtype=self.dtype))
        # -------------------add------------------------
        ...
        core_codes.append(neuronal_mix(mix_spike='mix_spike_seq[t]', spike='spike_seq[t]', real_spike='real_spike_seq[t]',
                                       alpha='alpha', dtype=self.dtype))

        self._core = core_codes.codes
        return self._core

class STHNeuronBPTTKernel(base.CKernel2D):
    def __init__(self, surrogate_function: Callable, hard_reset: bool, detach_reset: bool, dtype: str):
        ...
        self.add_param(ctype=f'{dtype} &', cname='v_th')
        # ------------------------add----------------------------
        # self.add_param(ctype=f'const {dtype} *', cname='real_spike_seq')
        # -------------------------------------------------------
        if hard_reset:
            self.add_param(ctype=f'{dtype} &', cname='v_reset')
    ...
    def core(self):
        ...
        # -----------------------------------add-----------------------------------------
        # core_codes.append(self.surrogate_function(y=f'const {self.dtype} grad_s_to_h', real_spike_seq='real_spike_seq[t]', dtype=self.dtype))
        core_codes.append(self.surrogate_function(x='over_th', y=f'const {self.dtype} grad_s_to_h', dtype=self.dtype))
        # -------------------------------------------------------------------------------
...
class STHNeuronATGFBase:
    ...
    def pre_forward(py_dict: dict):
        ...
        # -------------------add----------------------------------
        new_tensors(('h_seq', 'spike_seq', 'v_seq', 'real_spike_seq', 'mix_spike_seq'), py_dict)
        # -------------------add----------------------------------
    ...
    def pre_backward(ctx, grad_spike_seq: torch.Tensor, grad_v_seq: torch.Tensor):
        ...
        v_reset = ctx.v_reset
        # --------add--------------
        # real_spike_seq = ctx.real_spike_seq
        # --------add--------------
        ...
        py_dict = {
            ...
            'v_th': v_th,
            'v_reset': v_reset,
            # 'real_spike_seq': real_spike_seq
        }

        return backward_kernel, blocks, threads, py_dict

class STHLIFNodeFPTTKernel(STHNeuronFPTTKernel):
    ...
    def neuronal_charge(self) -> str:
        if self.hard_reset:
            codes = cfunction.sub(z=f'{self.dtype} STHLIFNodeFPTTKernel_temp_var', x='v_v_seq[t]', y='v_reset', dtype=self.dtype)
        else:
            codes = f'{self.dtype} STHLIFNodeFPTTKernel_temp_var = v_v_seq[t];'

        if self.decay_input:
            codes += cfunction.sub(z='STHLIFNodeFPTTKernel_temp_var', x='x_seq[t]', y='STHLIFNodeFPTTKernel_temp_var', dtype=self.dtype)
            codes += cfunction.mul(z='STHLIFNodeFPTTKernel_temp_var', x='decay', y='STHLIFNodeFPTTKernel_temp_var', dtype=self.dtype)
        else:
            codes += cfunction.mul(z='STHLIFNodeFPTTKernel_temp_var', x='decay', y='STHLIFNodeFPTTKernel_temp_var',
                                   dtype=self.dtype)
            codes += cfunction.sub(z='STHLIFNodeFPTTKernel_temp_var', x='x_seq[t]', y='STHLIFNodeFPTTKernel_temp_var',
                                   dtype=self.dtype)

        codes += cfunction.add(z='h_seq[t]', x='STHLIFNodeFPTTKernel_temp_var', y='v_v_seq[t]', dtype=self.dtype)

        return codes

class STHLIFNodeBPTTKernel(STHNeuronBPTTKernel):
...
class STHLIFNodeATGF(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x_seq: torch.Tensor, alpha: float, v_init: torch.Tensor, v_th: float, v_reset: float or None, decay: float,
                forward_kernel: STHLIFNodeFPTTKernel, backward_kernel: STHLIFNodeBPTTKernel):
        py_dict = {
            'x_seq': x_seq,
            # ------add------
            'alpha': alpha,
        ...
        requires_grad, blocks, threads, py_dict = STHNeuronATGFBase.pre_forward(py_dict)
        ...
        # qlNeuronATGFBase.ctx_save(ctx, requires_grad, py_dict['h_seq'], blocks=blocks, threads=threads,
        #                    numel=py_dict['numel'], N=py_dict['N'], v_th=py_dict['v_th'], v_reset=py_dict['v_reset'],
        #                    real_spike_seq=py_dict['real_spike_seq'], backward_kernel=backward_kernel, decay=py_dict['decay'])

        # 保存前向值,反向使用
        STHNeuronATGFBase.ctx_save(ctx, requires_grad, py_dict['h_seq'], blocks=blocks, threads=threads,
                           numel=py_dict['numel'], N=py_dict['N'], v_th=py_dict['v_th'], v_reset=py_dict['v_reset'],
                                  backward_kernel=backward_kernel, decay=py_dict['decay'])

        return py_dict['mix_spike_seq'], py_dict['v_v_seq'][1:, ]
        # return py_dict['spike_seq'], py_dict['v_v_seq'][1:, ]

    @staticmethod
    def backward(ctx, grad_spike_seq: torch.Tensor, grad_v_seq: torch.Tensor):

        backward_kernel, blocks, threads, py_dict = STHNeuronATGFBase.pre_backward(ctx, grad_spike_seq, grad_v_seq)
        ...
        return py_dict['grad_x_seq'], None, py_dict['grad_v_init'], None, None, None, None, None